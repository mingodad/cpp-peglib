...
simple_stmt = small_stmt (_ ';' _ small_stmt)* (_ ';')? _ EOL
small_stmt <-
  / assignment
  / 'pass' &(SPACE / ';' / EOL)
  / 'break' &(SPACE / ';' / EOL)
...
augassign = <
  '+' / '-' / '**' / '*' / '@' / '//' / '/' / '%' / '&' / '|' / '^' / '<<' / '>>'
>
global_stmt = 'global' SPACE (NAME (_ ',' _ NAME)* (_ ',')?)
nonlocal_stmt = 'nonlocal' SPACE (NAME (_ ',' _ NAME)* (_ ',')?)
yield_stmt <-
...
import_name = 'import' SPACE (as_name (_ ',' _ as_name)*)
as_name = dot_name (SPACE 'as' SPACE NAME)?
import_from <-
  / 'from' SPACE (_ <'.'>)* (_ dot_name)? SPACE 'import' SPACE '*'
  / 'from' SPACE (_ <'.'>)* (_ dot_name)? SPACE 'import' SPACE
    (from_as_parens / from_as_items)
from_as_parens = '(' _ (from_as (_ ',' _ from_as)* (_ ',')?) _ ')'
from_as_items = (from_as (_ ',' _ from_as)*)
...
from_params = '(' _ (from_param (_ ',' _ from_param)* (_ ',')?)? _ ')'
...
with_parens_item = '(' _ (as_item (_ ',' _ as_item)* (_ ',')?) _ ')'
with_item = (as_item (_ ',' _ as_item)*)
as_item <-
...
base_class_args = '(' _ (expression (_ ',' _ expression)* (_ ',')?)? _ ')'
class_args <-
...
expressions = (expression (_ ',' _ expression)* (_ ',')?)
expression <-
  / lambdef
...
lambdef <-
  / 'lambda' SPACE (NAME (_ ',' _ NAME)*) _ ':' _ expression
...
tuple <-
  / '(' _ ')'
  / '(' _ (star_named_expression (_ ',' _ star_named_expression)* (_ ',')?) _ ')'
yield = '(' _ 'yield' _ ')'
...
star_targets = (star_target (_ ',' _ star_target)* (_ ',')?)
star_target <-
  / '*' _ !'*' star_target
  / star_parens
  / primary
star_parens <-
  / '(' _ (star_target (_ ',' _ star_target)* (_ ',')?) _ ')'
  / '[' _ (star_target (_ ',' _ star_target)* (_ ',')?) _ ']'

star_expressions = (star_expression (_ ',' _ star_expression)* (_ ',')?)
star_expression <-
  / '*' _ bitwise_or
  / expression
...
named_expression <-
  / NAME _ ':=' _ #^ expression
  / expression !(_ ':=')
arguments = '(' _ (args (_ ',' _ args)* (_ ',')?)? _ ')'
args = (simple_args (_ ',' _ kwargs)? / kwargs)
#simple_args = list(',', (starred_expression / named_expression !(_ '=')))
simple_args = (starred_expression / named_expression !(_ '=')) (',' (starred_expression / named_expression !(_ '=')))*
starred_expression = '*' _ expression
kwargs <-
  / (kwarg_or_starred (_ ',' _ kwarg_or_starred)*) _ ',' _ (kwarg_or_double_starred (_ ',' _ kwarg_or_double_starred)*)
  / (kwarg_or_starred (_ ',' _ kwarg_or_starred)*)
  / (kwarg_or_double_starred (_ ',' _ kwarg_or_double_starred)*)
kwarg_or_starred <-
  / NAME _ '=' _ expression
  / starred_expression
